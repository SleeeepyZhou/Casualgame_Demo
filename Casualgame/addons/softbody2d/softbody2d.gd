...
func _ready():
	...
	for node in get_children():
		node.connect("child_order_changed",_node_removed)
	self.connect("child_order_changed",exit_scene)

func destroy(body:PhysicsBody2D):
	var rigid_body : SoftBodyChild = _soft_body_rigidbodies_dict[body]
...
func _node_removed():
	for body in get_rigid_bodies():
		if body.rigidbody != null and body.rigidbody.get_child_count() == 2:
			body.rigidbody.queue_free()
